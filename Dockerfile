FROM jupyter/base-notebook:2023-05-08 as jupyter-insarviz
...
RUN cd /opt/install/jupyter-x11-app-proxy \
 ...
 && pip install -e .
RUN mamba env update --name base --file /opt/install/insarviz/environment.yaml \
 && pip install --no-deps -e /opt/install/insarviz
